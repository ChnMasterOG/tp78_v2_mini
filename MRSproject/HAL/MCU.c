...
#if !(defined HAL_TPM) || (HAL_TPM == FALSE)
/* HID data */
...
UINT8* HIDMouse = NULL;
/* 键盘数据 */
UINT8* HIDKeyboard = NULL;
/* 音量控制数据 */
UINT8* HIDVolume = NULL;
/* HID ready control, index 0-slave, index 1-master */
UINT8* HID_Keyboard_ReadyCTL = NULL;
UINT8* HID_Mouse_ReadyCTL = NULL;
UINT8* HID_Vol_ReadyCTL = NULL;
#endif
...
//      }
      ...
      if ( KEYBOARD_Custom_Function() ) { // 普通按键,带有Fn键处理信息则不产生键盘事件
        ...
        *HID_Keyboard_ReadyCTL = MODULE_BIT_KEYBOARD_READY;
#endif
      }
...
#if (defined HAL_TPM) && (HAL_TPM == TRUE) && (defined HAL_HW_I2C) && (HAL_HW_I2C == TRUE)
        *HID_Mouse_ReadyCTL = MODULE_BIT_MOUSE_READY;
#endif
      }
    }
  }
...
#if (defined HAL_TPM) && (HAL_TPM == TRUE) && (defined HAL_HW_I2C) && (HAL_HW_I2C == TRUE)
  *HID_Vol_ReadyCTL = MODULE_BIT_VOL_READY;
#endif
}
...
//  static uint8_t tpm_cnt = 0;
  ...
  if ( events & MAIN_CIRCULATION_EVENT)
  {
    ...
    if (!TPM_get_TP78_alive())  // 主键盘连接下lowpower状态随主键盘控制
#endif
      ++idle_cnt;
#if (defined HAL_TPM) && (HAL_TPM == TRUE)
    TPM_loss_TP78_connection();
    if (TPM_get_backlight_change((uint8_t *)&g_LED_Style) == TRUE) {
    ...
    if (TPM_get_sleep_status_change(&g_Sleep_Status) == TRUE) {
      if (g_Sleep_Status == 0) {
        TP78Reinit(1, lp_sw_mode);
      } else if (g_Sleep_Status == 2) {
        TP78Reinit(0, lp_sw_mode);
      }
    }
#endif
  ...
  if ( events & WS2812_EVENT )
  {
    ...
    HW_WS2812_Process();
#endif
    tmos_start_task( halTaskID, WS2812_EVENT, MS1_TO_SYSTEM_TIME(85) ); // 80ms周期控制背光
