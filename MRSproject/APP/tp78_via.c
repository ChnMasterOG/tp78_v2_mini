...
void via_data_processing(uint8_t *data, uint8_t len)
{
  ...
  switch (*command_id) {
    ...
    case (uint8_t)VIA_ID_DYNAMIC_KEYMAP_GET_KEYCODE: {
        ...
        **************************/
        ...
        if (command_data[0] == 0) { // layer 0
          ...
          if (Extra_CustomKey[command_data[0]-1][command_data[1]][command_data[2]] >= KEY_SP_1 && Extra_CustomKey[command_data[0]-1][command_data[1]][command_data[2]] <= KEY_SP_7) {
            command_data[3] = 0x77;
            command_data[4] = Extra_CustomKey[command_data[0]-1][command_data[1]][command_data[2]] - KEY_SP_1;
          } else {
            command_data[4] = Extra_CustomKey[command_data[0]-1][command_data[1]][command_data[2]];
          }
        }
        break;
    }
    case (uint8_t)VIA_ID_DYNAMIC_KEYMAP_SET_KEYCODE: {
        ...
        **************************/
        ...
        if (command_data[0] == 0) { // layer 0
          ...
          if (command_data[3] == 0x77)  // SP_KEY
            Extra_CustomKey[command_data[0]-1][command_data[1]][command_data[2]] = KEY_SP_1 + command_data[4];
          else
            Extra_CustomKey[command_data[0]-1][command_data[1]][command_data[2]] = command_data[4];
          sprintf(buf, "0:keyboard_ext_%d_mat.txt", command_data[0]-1);
          HAL_Fs_Write_keyboard_mat(buf, (const uint8_t*)Extra_CustomKey[command_data[0]-1]);
        }
        break;
    }
    case (uint8_t)VIA_ID_DYNAMIC_KEYMAP_RESET: {
        memcpy(CustomKey, KeyArrary, COL_SIZE*ROW_SIZE);
        memset(Extra_CustomKey, KEY_None, LAYER_SIZE*COL_SIZE*ROW_SIZE);
    ...
    case (uint8_t)VIA_ID_DYNAMIC_KEYMAP_GET_LAYER_COUNT: {
        command_data[0] = LAYER_SIZE + 1;  // layer 0 - main, layer 1~4 - extra
        break;
    }
    case (uint8_t)VIA_ID_DYNAMIC_KEYMAP_GET_BUFFER: {
        ...
        layer_idx = offset / (COL_SIZE * ROW_SIZE);
        if (layer_idx == 0) { // layer 0
